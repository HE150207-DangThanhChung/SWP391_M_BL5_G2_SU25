                                ...
                                <div class="card shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" step="0.01" class="form-control" id="price" name="price" value="${formattedPrice}" required>
                                        ...
                                        <div class="spec-container">
                                            <%-- Loop through ONLY the attributes assigned to this specific variant --%>
                                            <c:forEach var="variantAttr" items="${variant.attributes}">
                                                <div class="form-group">
                                                    <%-- CORRECTED PATH: Get the attribute's name directly from the AttributeOption --%>
                                                    <label>${variantAttr.attribute.attributeName}</label>
                                                    <select class="form-control" name="attributeOptionId[]" required>
                                                        <%-- CORRECTED PATH --%>
                                                        <option value="">Chọn ${variantAttr.attribute.attributeName}</option>

                                                        <%-- CORRECTED PATH: Get all possible options for this attribute type --%>
                                                        <c:forEach var="option" items="${attributeOptions[variantAttr.attribute.attributeId]}">
                                                            <%-- Select the option that is already saved for this variant (this line was already correct) --%>
                                                            <option value="${option.attributeOptionId}" ${variantAttr.attributeOptionId == option.attributeOptionId ? 'selected' : ''}>
                                                                ${option.value}
                                                            </option>
                                                        </c:forEach>
                                                    </select>
                                                    <%-- CORRECTED PATH --%>
                                                    <div class="error-message">Vui lòng chọn ${variantAttr.attribute.attributeName.toLowerCase()}</div>
                                                </div>
                                            </c:forEach>
                                        </div>
